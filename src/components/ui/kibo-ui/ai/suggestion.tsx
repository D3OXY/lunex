...
export const AISuggestion = ({ suggestion, onClick, className, variant = "outline", size = "sm", children, ...props }: AISuggestionProps) => {
    const handleClick = () => {
        onClick?.(suggestion);
    };

    return (
        <Button type="button" variant={variant} size={size} className={cn("cursor-pointer rounded-full px-4", className)} onClick={handleClick} {...props}>
            {children || suggestion}
        </Button>
    );
};
